...
> **Version.** Lite content **3.2-PROPOSED**-standalone · kit **5.3.0** (+ Ultra from ControlPlane — REVIEW BEFORE PROMOTE)  
...
1. Create folder structure for chosen tier using **Core spine names** (orchestrator/, .specify/, plan/, work/, protocols/, MASTER-FLOW.md) — thin files OK, renamed spine NOT OK
...
# ═══════════════════════════════════════════════════════════
# ULTRA ADDENDUM — 3.2-PROPOSED (from ControlPlane / SEBI live use)
# Additive. If anything conflicts with §0–§4 above, KEEP original and file an issue.
# Promote only after human review. Do not delete prior sections.
# ═══════════════════════════════════════════════════════════

## U0. Lite ↔ Core parity spine (mandatory naming)

Lite may be **thinner** (fewer files inside folders) but MUST use the **same hierarchy names** as Core emits:

```
AGENTS.md · HANDOFF.md · HIERARCHY.md · HOW_TO_RUN.md · MASTER-FLOW.md
orchestrator/     # Tier-1 — never rename to "meta/" or "brain/"
.specify/         # constitution + specs/wave-N
plan/             # PRD · ARCHITECTURE · EXECUTION
work/             # wave-N tasks + reports/wave-N
protocols/        # spine copies (or kernel) + project extras
prompts/current/
evals/            # stub OK at T1
```

**Thin ≠ different shape.** Calling a random folder layout “Adaptoid” is FM-16 wrong-route.

## U1. Master paste protocol (workers)

Workers receive **exactly**:
1. `work/WORKER_PROMPT.md` (≤15 laws), then
2. **One** path: `work/<wave>/<task>.md`

No multi-page re-brief. No chat history required. Report → `work/reports/<wave>/<id>.report.md`.

## U2. Task file required fields
...
## Acceptance
- [ ] …
- [ ] evidence command pasted in report

## Commit message
`type: summary`

## Report
`work/reports/wave-N/<ID>.report.md`
```

Missing `writes`/`forbid` → invalid task (blocks parallel dispatch).

## U3. Parallel map (extends FM-13)

Orchestrator publishes batches. Same file path → serialize **or** git worktree.  
Preflight **fails** on tracked `* 2.md` / `* 2.*` (Finder/IDE collision artifacts).

## U4. Domain invariants block (AGENTS.md)

Separate from stack. Examples from competition products:
- Content laws / fail-closed / “never say X”
- No fake metrics / logos
- Critical path constraints (e.g. no LLM on Lane 1)

Workers must not “improve” invariants.

## U5. Confidence ≠ completeness

Ship close requires an **acceptance matrix** (`docs/ACCEPTANCE.md` or equivalent):
- git SHA
- exact commands
- expected outputs
- pasted evidence

Feeling “1% done” or “100% done” is not status. EXECUTION.md SHIPPED rows need commit hashes.

## U6. Evidence pack wave (final)

Last wave always includes: acceptance · audit · assumptions · hostile/kill-shot · event checklist · tag **recipe** (human creates tag).

## U7. Orchestrator must not implement product (FM-26)

Exception: Phase-0 OS scaffold only. Feature code → workers. Orchestrator: dispatch, review, merge, verify.

## U8. Archetype: competition / hackathon

When brief signals prize/pitch/judge demo:
- Generate judge runbook + hostile Q + event checklist
- Domain invariants in AGENTS.md
- Offline-safe demo path
- Never invent FNR% / wrong latency percentile / fake logos

## U9. Proposed new failure modes (Core library)

| ID | Name |
|---|---|
| FM-22 | False confidence (feeling ≠ ship state) |
| FM-23 | Parallel dupe files (`foo 2.md`) |
| FM-24 | EXECUTION drift |
| FM-25 | Domain invariant break |
| FM-26 | Orchestrator implements (two-tier collapse) |

Full text: Core `improvements/failure-modes/` until promoted into `failure-modes/`.

## U10. Proposed protocols (Core)

P-06 writes/forbid · P-07 master paste · P-08 acceptance matrix · P-09 domain invariants · P-10 EXECUTION drift gate · P-11 evidence pack wave · P-12 dupe quarantine  

Full text: `improvements/ULTRA-PROTOCOLS.md` / `improvements/protocols/`.

## U11. How to use 3.2-PROPOSED safely

| Mode | What to paste |
|---|---|
| Stable | Original `ADAPTOID-LITE.md` 3.1 only |
| Ultra trial | This entire `ADAPTOID-LITE-3.2-PROPOSED.md` |
| Additive trial | 3.1 + sections U0–U11 only |

After trial, promote accepted bullets into official 3.2 and bump CHANGELOG on Adaptoid-OS.
